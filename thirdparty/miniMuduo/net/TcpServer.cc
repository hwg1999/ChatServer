...
TcpServer::TcpServer(EventLoop *loop, const InetAddress &listenAddr,
    ...
    : loop_(CheckLoopNotNull(loop)),
      ...
      acceptor_(new Acceptor(loop, listenAddr,
                             option == kReusePort)), // 处理新连接的Acceptor
...
void TcpServer::newConnection(int sockfd, const InetAddress &peerAddr) {
  ...
  LOG_DEBUG(
      "func=%s => TcpServer::newConnection [%s] - [%s] from %s ",
      __FUNCTION__, name_.c_str(), connName.c_str(),
...
void TcpServer::removeConnectionInLoop(const TcpConnectionPtr &conn) {
  LOG_DEBUG("func=%s => TcpServer::removeConnectionInLoop [%s] - connection %s",
            __FUNCTION__, name_.c_str(), conn->name().c_str());
