      ...
      peerAddr_(peerAddr), highWaterMark_(64 * 1024 * 1024) // 缓冲区数据最大64M
{
  ...
  LOG_DEBUG("func=%s => TcpConnection::ctor[%s] at fd=%d", __FUNCTION__,
            name_.c_str(), sockfd);
  // 设置保活机制
...
TcpConnection::~TcpConnection() {
  LOG_INFO("Connection Down! [%s] at fd=%d state=%d", name_.c_str(),
...
void TcpConnection::handleClose() {
  LOG_DEBUG("func=%s => TcpConnection::handleClose fd=%d state=%d ",
            __FUNCTION__, channel_->fd(), (int)state_);
